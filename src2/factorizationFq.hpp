 ...
 * */
    ...
    std::vector<std::pair<Fxelem, unsigned int> > partialFactorDD(Fxelem pol) {
		...
		if (salir) 
			std::cout<< "Las matrices no coinciden" << std::endl;
 ...
 */
    ...
    matrix<typename Fxelem::Felem> formMatrixBigQ(const Fxelem &pol) {
       ...
       result.push_back(static_cast<std::vector<Fpelem_b>>(pwrsX[0])); //x^n mod pol
       Fxelem aux = pwrsX[0];
        for (int i = 2; i <= (int)(polDeg) - 1; ++i) {
			 aux = aux*pwrsX[0]; //(x^{i*n});
			...
			std::cout <<aux << std::endl;
			result.push_back(static_cast<std::vector<Fpelem_b>>(aux));
		}
        return result;
	}
